defmodule Aiex.CLI.Commands.HelpTest do
  ...
  describe "execute/1" do
    test "shows general help for empty args" do
      ...
      assert {:ok, {:info, "Aiex - AI-powered Elixir coding assistant", info}} =
               Help.execute(input)
    ...
    test "shows general help for help command without args" do
      ...
      assert {:ok, {:info, "Aiex - AI-powered Elixir coding assistant", _info}} =
               Help.execute(input)
    end

    test "shows command-specific help for create command" do
      input =
        {[:help], %Optimus.ParseResult{args: %{command: "create"}, flags: %{}, options: %{}}}
    ...
    test "shows command-specific help for analyze command" do
      input =
        {[:help], %Optimus.ParseResult{args: %{command: "analyze"}, flags: %{}, options: %{}}}
    ...
    test "shows error for unknown command help" do
      input =
        {[:help], %Optimus.ParseResult{args: %{command: "unknown"}, flags: %{}, options: %{}}}

      assert {:error, "Unknown command: unknown. Use 'aiex help' for available commands."} =
               Help.execute(input)
    end
  end
end
