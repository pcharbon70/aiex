defmodule Mix.Tasks.Ai.Explain do
  ...
  defp explain_file(file_path, function_name, opts) do
    Mix.shell().info(
      "🤖 Analyzing #{file_path}#{if function_name, do: " (function: #{function_name})", else: ""}..."
    )
  ...
  def find_function_bounds(lines, function_name) do
    ...
    if start_index do
      ...
      end_offset =
        Enum.find_index(remaining_lines, fn line ->
  ...
  defp build_explanation_request(code, source, function_name, opts) do
    ...
    focus_instruction =
      case focus_area do
        "architecture" ->
          "Focus on architectural patterns, design decisions, and module organization."

        "patterns" ->
          "Focus on Elixir patterns, idioms, and functional programming concepts used."

        "performance" ->
          "Focus on performance characteristics, potential bottlenecks, and optimizations."

        "security" ->
          "Focus on security considerations, input validation, and potential vulnerabilities."

        _ ->
          "Provide a comprehensive explanation covering all relevant aspects."
      end
    ...
    %{
      messages: [
        %{
          role: :system,
          content:
            "You are an expert Elixir developer and teacher. Provide clear, accurate explanations of Elixir code."
        },
