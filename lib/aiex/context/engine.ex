defmodule Aiex.Context.Engine do
  ...
  @max_hot_tier_size 100_000_000  # 100MB
  @max_warm_tier_size 500_000_000  # 500MB
  ...
  def init(opts) do
    ...
    table = :ets.new(:aiex_context_table, [
      :set,
      :public,
    ...
    dets_ref = make_ref()
    {:ok, dets_table} = :dets.open_file(dets_ref, [
  ...
  defp persist_to_dets(state) do
    ...
    :ets.foldl(fn {key, entry}, _acc ->
      :dets.insert(state.dets_table, {key, entry})
      :ok
    end, :ok, state.ets_table)
  ...
  defp update_tier_tracking(state, key, entry) do
    ...
    state = Enum.reduce([:hot_tier, :warm_tier, :cold_tier], state, fn tier, acc ->
  ...
  defp manage_memory_tiers(state) do
    ...
    state = if hot_size > @max_hot_tier_size do
  ...
  defp demote_from_warm_tier(state) do
    ...
    warm_entries = state.warm_tier
    |> Enum.map(fn {key, entry} -> 
      {key, entry, Map.get(state.last_accessed, key, 0)}
    end)
    |> Enum.sort_by(fn {_, _, last_accessed} -> last_accessed end)
    |> Enum.take(div(map_size(state.warm_tier), 4)) # Demote 25%
