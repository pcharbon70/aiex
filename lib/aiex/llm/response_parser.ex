defmodule Aiex.LLM.ResponseParser do
  ...
  def extract_sections(content) do
    ...
    sections = Regex.scan(~r/^#+\s*(.+?)$(.*?)(?=^#+|\z)/sm, content, capture: :all_but_first)
  ...
  def validate_code(code, language) do
    case language do
      ...
      _ -> :ok  # Skip validation for unsupported languages
    end
  end
  ...
  defp parse_test_generation(content, opts) do
    ...
    test_blocks = Enum.filter(code_blocks, fn block ->
  ...
  defp find_main_code_block(blocks, language) do
    ...
    language_match = Enum.find(blocks, fn block -> 
