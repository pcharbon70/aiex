defmodule Aiex.Sandbox.AuditLogger do
  ...
  """
  @spec log_access_attempt(String.t(), atom(), atom() | {atom(), any()}, Aiex.Sandbox.Config.t()) :: :ok
  ...
  """
  @spec log_operation_result(String.t() | {String.t(), String.t()}, atom(), atom() | {atom(), any()}, Aiex.Sandbox.Config.t()) :: :ok
  ...
  def handle_call({:get_recent, count}, _from, state) do
    recent = :queue.to_list(state.recent_entries)
    |> Enum.take(-count)
  ...
  def handle_call({:search, criteria}, _from, state) do
    ...
    results = :queue.to_list(state.recent_entries)
  ...
  defp should_log?(event_type, config) do
    config.audit_enabled and case config.audit_level do
  ...
  defp buffer_event(formatted_event, raw_event, state) do
    ...
    new_recent = if :queue.len(new_recent) > state.max_recent do
  ...
  defp flush_buffer(state) do
    if state.buffer != [] do
      ...
      new_state = if state.current_size + byte_size(content) > @max_log_size do
        rotate_logs(state)
      else
        state
      end
      ...
      %{new_state | 
        buffer: [],
        current_size: new_state.current_size + byte_size(content)
      }
    else
      state
    end
  ...
  defp schedule_flush do
    Process.send_after(self(), :flush, 5_000)  # Flush every 5 seconds
  end
