defmodule Aiex.NATS.ServerManager do
  ...
  defp build_config(opts) do
    ...
    %{
      ...
      binary_path:
        Keyword.get(config, :binary_path, Path.join([priv_dir, "nats", "bin", "nats-server"])),
      config_path:
        Keyword.get(config, :config_path, Path.join([priv_dir, "nats", "config", "server.conf"])),
      ...
      case :erlang.system_info(:system_architecture) do
        arch when is_list(arch) ->
          ...
          cond do
            ...
            String.contains?(arch_str, "armv7") ->
              "arm7"

            String.contains?(arch_str, "386") ->
              "386"
  ...
  defp download_file(url, dest_path) do
    ...
    case :httpc.request(
           ...
           [{:body_format, :binary}, {:stream, dest_path}]
         ) do
      {:ok, :saved_to_file} ->
        :ok
  ...
  defp extract_nats_binary(tar_path, dest_path) do
    ...
    result =
      case System.cmd("tar", ["-xzf", tar_path, "-C", extract_dir, "--strip-components=1"]) do
        {_, 0} ->
  ...
  defp start_nats_server(state) do
    ...
    args = [
      "-c",
      state.config_path,
      # Debug and verbose for development
      "-DV"
    ]
