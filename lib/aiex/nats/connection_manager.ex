defmodule Aiex.NATS.ConnectionManager do
  ...
  def handle_info(:connect, state) do
    case connect_to_nats(state.gnat_config) do
      {:ok, pid} ->
        ...
        {:noreply, %{state | connection_pid: pid, status: :connected, reconnect_attempts: 0}}
      ...
      {:error, reason} ->
        if state.reconnect_attempts == 0 do
          ...
          Logger.debug(
            "NATS connection attempt #{state.reconnect_attempts + 1} failed: #{inspect(reason)}"
          )
        end
  ...
  def handle_info({:DOWN, _ref, :process, pid, reason}, %{connection_pid: pid} = state) do
    ...
    {:noreply, %{state | connection_pid: nil, status: :disconnected}}
  end
