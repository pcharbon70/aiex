defmodule Aiex.TUI.Server do
  ...
  def handle_call(:list_clients, _from, state) do
    clients = Enum.map(state.clients, fn {id, client} ->
      %{
        id: id,
  ...
  def handle_info({:tcp, socket, data}, state) do
    ...
    client_id = Enum.find_value(state.clients, fn {id, %{socket: s}} ->
      if s == socket, do: id
    end)
  ...
  defp handle_client_message(client_id, %{"type" => "request", "id" => request_id} = message, state) do
  ...
  defp remove_client_by_socket(socket, state) do
    case Enum.find(state.clients, fn {_id, %{socket: s}} -> s == socket end) do
      {client_id, client} ->
        ...
        new_subscriptions = Enum.reduce(client.subscriptions, state.subscriptions, fn topic, subs_map ->
